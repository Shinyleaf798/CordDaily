...
`(tabs)/settings.tsx` 从一长条卡片列表改成四个圆形图标（账本 / 布局 / 主题 / 其他），
每格跳一个二级页：`settings/ledger`、`settings/home-layout`、`settings/theme`、`settings/other`。
账号信息和退出登录挪进了「其他」。主题页每项左边多了一条色板预览。

**为什么这样设计**

原来那一页把四类互不相干的设置堆在一个滚动列表里，越往下加东西越难找。
网格的优势不是好看，是**四个选项一屏内全部可见、不用滚动就能扫完**——跟记账页的分类网格是同一个道理，
都是"扫一眼挑一个"的场景，长列表读起来更慢。

格子宽度用 25% 而不是分类网格的 20%：分类有十几个，20% 才排得下一行五个；
这里只有四个，25% 正好铺满一行，不会在右边留一截空。

四个入口都开路由而不是弹层，判断标准是 `modal-host.tsx` 里那条**「这一层有没有自己的地址」**：
它们可以被直接跳进来、该出现在返回栈里、按返回键该退回「我的」，而且不用往回传值。

主题页的色板预览：三套主题真正的差别就是 `cardHighlight` 那一个颜色，
光看「黑金 / 黑紫」这种名字选不出来，得让人看见它长什么样。

**放弃了什么**

没有把邮箱留在「我的」顶部当身份标识。邮箱一年确认一次、登出更少，
留在首屏等于用最显眼的位置放最少用的东西。

`settings/home-layout.tsx` 没有命名成 `layout.tsx`——虽然合法（`_layout.tsx` 才是布局文件），
但两个名字挨着太容易看串。
...
三处 `slice` 改掉，并在 `utils/date.ts` 新增 `formatMonthKey`：

- `use-transactions.ts` / `use-budgets.ts` 的 `monthKey`：`toISOString().slice(0, 7)` → `formatMonthKey()`
- `reimbursements.tsx` 的日期显示：`item.date.slice(0, 10)` → `formatMonthDay(new Date(item.date))`

**为什么**

`toISOString()` 永远把时刻渲染成 **UTC**。东八区比 UTC 快 8 小时，所以**本地 00:00–08:00 这段时间里
UTC 还停在昨天**，切出来的年月日是「UTC 眼里的那一天」。

两种切法的严重程度差很多：
- `slice(0, 10)` 取日 → **每天都错 8 小时**，1/3 的时间
- `slice(0, 7)` 取月 → 只有每月 1 号的 00:00–08:00

所以报销清单那处是真被看见的 bug（凌晨记的账全显示成前一天），
而 `monthKey` 那两处的症状是 9 月 1 号凌晨首页先闪一下 8 月的汇总——
因为那 8 小时里算出来的 key 跟上个月的 key 一模一样，React Query 直接命中了上个月的缓存。

更要命的是**切完就救不回来了**：完整的 `2026-08-31T19:00:00.000Z` 里既有时刻也有基准（`Z`），
转到任何时区都行；切成 `2026-08-31` 之后 19:00 和 `Z` 都没了，而进不进位恰恰由那个 19:00 决定。

规矩一句话：**ISO 字符串是给机器存的，`Date` 的取值方法是给人看的，中间那一步不能省。**

**为什么之前没抓到**

`utils/date.ts` 顶上早就写了这条警告，但只点名了 `slice(0,10)`。
`slice(0,7)` 是同一个坑换了个切法，又藏在"缓存 key"这种看不见的地方，就溜过去了。
`formatMonthKey` 的注释里现在两种切法都点名了。

## 2026-09-18 二级分类，以及分类网格里两个"等分"踩的坑

**改了什么**

- `categories` 表 v1 就有 `parentId`，一直没人用。现在网格只画一级分类，有子级的格子右下角带一颗 `···` 点，点开在它下面浮出一个二级面板（独立 Modal，能盖住分类卡以外的东西）。选中子分类后那一格显示「餐饮·晚餐」。
- 面板第一格是父分类自己，点面板外面也等于选中父分类——否则有子分类的一级分类永远选不中。
- 分类管理页支持在一级下面新建子分类；有子分类的一级删不掉（不做级联）。
- 默认灌 15 个子分类，用 `app_settings` 里的标记位控制只跑一次——不能用 `seedDefaultCategories` 那套"表空才跑"，老用户的分类表早就有数据了。

**两个"等分"的坑，同一个根**

**① `width: '20%'` 排不下 5 个。** 1/5 在某些屏宽上除不尽，每格四舍五入到整数像素后 5 格加起来超过容器宽度，第 5 格被挤到下一行——真机上表现为"一行只剩 4 个"。日历那边 `1/7` 踩过完全一样的坑（表头变成"日一二三四五 / 六"）。

改成切行渲染：一行一个真的 `<View flexDirection:'row'>`，每格 `flex: 1`。flex 分的是剩余像素，不存在舍入溢出。最后一行补空格子，否则 `flex: 1` 会把剩下那两三个摊开占满整行。

**规矩：要等分就别用百分比 + flexWrap，用真的行 + flex。**

**② 选中的圆在真机上画成方块。**

A/B 实测过（两版只差这一处，行布局完全相同）：把 `borderRadius` + `backgroundColor` 写在**装着图标和那颗点**的容器上 → 方块；换成一个独立的空 View → 正圆。

原因：`borderRadius` 的含义是"把这个 View 连同里面的内容一起裁成圆角"。而那颗 `···` 点用 `right/bottom: -2` **故意伸出边界**，一裁就会被削掉。系统不裁了——不裁就等于没有圆角，背景退回矩形。

注意限定：**圆角 View 可以装东西，只是装的东西不能越界**。图标待在里面完全没事，问题只出在那颗骑在边上的点。
...
**规矩：一个 View 要圆角，里面的东西就得待在圆里；有东西非要伸出去，就把圆角挪到另一个 View 上。**

**为什么这个 bug 查了很久**

它有三层伪装，每一层都指向错误的方向：

- **"存盘时是圆的，点一下别的分类就变方"** → 像是运行时重建了背景。其实形状从第一帧就是方的，只是未选中时背景透明、看不见；点击只改了一个颜色值，什么结构都没动。
- **"二级面板里是圆的，网格里是方的"** → 像是两边样式分叉了。其实两边用的是同一个样式，差别在面板里的格子**没有那颗点**。
- 选中的恰好是 📚 🏠 这类本身就方的 emoji，一度以为用户看到的是 emoji 自己。

真正定位它的是那句「点开有子级的，里面是圆的、关掉外面是方的」——那是唯一一次把"有没有那颗点"和"圆还是方"直接对上。教训是：**形状只跟结构有关，跟状态无关；一开始就该去比较两处的结构差异，而不是去找"点击时发生了什么"。**

**顺带**

网格和面板原本是两份几乎一样的 JSX，现在抽成同一个 `IconCell`。上面那个"面板圆、网格方"的迷惑，起因就是两边各写各的、结构悄悄分叉了。
